...
class OPENAI_GPT:
    ...
    def llm_inference(self, messages):
        ...
        # response = client.chat.completions.create(model=self.engine, # engine = "deployment_name".
        try:
            ...
            self.logger.error("Bad request error: messages: %s", messages)
            self.logger.error("Bad request error: error: %s", error)
            breakpoint()
        ...
        for attempt in range(self.max_retry_iters):
            try:
                ...
                if attempt < self.max_retry_iters - 1:  # If not the last attempt
                    sleep(self.retry_delays)  # Wait before retrying
