...
class Evalalfworld(BaseTask):
    ...
    def evaluate_env(self,  index, ob='', examples=None):
        ...
        trajectory = [{"Goal":goal, "id":0}, {"Observation":init_ob, "id":0}]
        # trajectory.append({"Goal":goal, "id":0})
        # trajectory.append({"Observation":init_ob, "id":0})   
        
        for i in (pbar := trange(self.max_num_steps, leave=False)):
            pbar.set_description(f"Evalalfworld.evaluate_env: Processing step i={i} out of {self.max_num_steps}")
            ...
            if "Task accomplished!" in observation and reward < 1.0:
                raise RuntimeError("Task accomplished error")
    ...
    def evaluate(self):
        ...
        evaluate_env = self.evaluate_env
        self_examples_examples = self.prompts['examples']
        
        # for idx in (pbar_outer := trange(self.num_exams, leave=False)):
        for idx in (pbar_outer := trange(self.num_exams, leave=False)):
            pbar_outer.set_description(f"Evalalfworld.evaluate: Processing exam idx={idx} out of {self.num_exams}")
            pbar_outer.write('Running reset')
            ob, info = self.env.reset()
            pbar_outer.write('Fininshed reset')
            ob = '\n'.join(ob[0].split('\n\n')[1:])
            name = '/'.join(info['extra.gamefile'][0].split('/')[-3:-1])
            #sub_goal = selected_obs[name]
            difficulties.append(self.env.difficulty)
            pbar_outer.write('Appended difficulty')

            for k, v in (pbar_inner := tqdm(PREFIXES.items(), leave=True)):
                pbar_inner.set_description(f"\tEvalalfworld.evaluate: Processing k={k}")
                if name.startswith(k):
                    examples = "".join(self_examples_examples[v])
                    score, is_done, grounding_acc, score_change_record, steps = evaluate_env(ob=ob, examples=examples, index=idx)
                    # if is_done:
                    #     srs.append(1.0)
                    # else:
                    #     srs.append(0.0)
                    ...
                    logger.finish(f"Example {idx} | Success: {is_done} , Progress Rate: {score} , Steps: {steps}\n")
