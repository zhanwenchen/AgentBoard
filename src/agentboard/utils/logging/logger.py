...
class SummaryLogger:
    ...
    def log_summary(self):
        ...
        N = len(CATEGORIES)
        result_df = pd.DataFrame(columns=["model_name", "task_name", "success_rate", "baseline"])
        for task_name in CATEGORIES:
            for model_name in all_results[task_name]:
                dash = False if model_name == "Current Run" else True
                result_df.loc[len(result_df)] = {"model_name": model_name, "task_name": task_name, "success_rate": 100 * all_results[task_name][model_name]["success_rate"], "baseline": dash}
        ...
        DIMENSION_CATEGORIES = self.dimension_scoring.keys()
        dimension_df = pd.DataFrame(columns=["model_name", "dimension", "score", "baseline"])
        ...
        for dimension in DIMENSION_CATEGORIES:
            ...
            for model_name in all_results[list(CATEGORIES)[0]]:
                ...
                dash = False if model_name == "Current Run" else True
                dimension_df.loc[len(dimension_df)] = {"model_name": model_name, "dimension": dimension, "score": score, "baseline": dash}
...
class TaskLogger:
    ...
    def log_summary(self, success_rate, reward_score, grounding_acc, score_steps, hard_sr=None, hard_rs=None, easy_sr=None, easy_rs=None):
        ...
        # draw a line plot in wandb for reward_wrt_steps
        df = pd.DataFrame(columns=["models", "steps", "score", "baseline"])
